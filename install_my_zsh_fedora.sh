...
if [[ "$answer" == "y" ]]; then
  ...
  if sudo dnf update; then
    if sudo dnf install git curl -y; then
  ...
  if git clone https://github.com/zsh-users/zsh-autosuggestions \ ${ZSH_CUSTOM:-"$HOME"/.oh-my-zsh/custom}/plugins/zsh-autosuggestions; then
  ...
  if git clone https://github.com/zsh-users/zsh-syntax-highlighting \ ${ZSH_CUSTOM:-"$HOME"/.oh-my-zsh/custom}/plugins/zsh-syntax-highlighting; then
